...
export type A11yScheme = (typeof ALL_SCHEME_CLASSES)[number] extends `a11y-scheme-${infer S}` ? S : never;
...
const ensurePlaceholder = (img: HTMLImageElement): void => {
	if ((img.nextElementSibling as HTMLElement | null)?.dataset?.a11yPlaceholder) return;
...
const applyToDOM = (s: A11ySettings): void => {
	...
	ALL_SCHEME_CLASSES.forEach((cls) => html.classList.remove(cls));
	if (s.enabled) {
